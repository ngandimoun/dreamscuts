...
export interface ImageReframeInput {
  image_url: string;
...
export interface ImageReframeOutput {
  images: Array<{
    url: string;
    content_type?: string;
    file_name?: string;
...
export class FalAiImageReframeExecutor {
  private modelId = "fal-ai/image-editing/reframe";

  /**
   * Reframe image with intelligent aspect ratio adjustment
   */
  async reframe(input: ImageReframeInput): Promise<ImageReframeOutput> {
    const params = {
    ...
    return result.data as ImageReframeOutput;
  }

  /**
   * Reframe to specific aspect ratio
   */
  async reframeToAspectRatio(input: ImageReframeInput & { aspect_ratio: "21:9" | "16:9" | "4:3" | "3:2" | "1:1" | "2:3" | "3:4" | "9:16" | "9:21" }): Promise<ImageReframeOutput> {
    return this.reframe(input);
  }

  /**
   * High-quality reframing with quality settings
   */
  async reframeHighQuality(input: ImageReframeInput & { 
    num_inference_steps: number; 
    guidance_scale: number 
  }): Promise<ImageReframeOutput> {
    return this.reframe(input);
  }

  /**
   * Fast reframing with performance settings
   */
  async reframeFast(input: ImageReframeInput & { 
    num_inference_steps: number; 
    guidance_scale: number 
  }): Promise<ImageReframeOutput> {
    return this.reframe(input);
  ...
  /**
   * Reframe for mobile format (9:16)
   */
  async reframeForMobile(input: ImageReframeInput & { aspect_ratio: "9:16" }): Promise<ImageReframeOutput> {
    return this.reframe(input);
  }

  /**
   * Reframe for social media square format (1:1)
   */
  async reframeForSocialMedia(input: ImageReframeInput & { aspect_ratio: "1:1" }): Promise<ImageReframeOutput> {
    return this.reframe(input);
  }

  /**
   * Reframe for widescreen format (16:9)
   */
  async reframeForWidescreen(input: ImageReframeInput & { aspect_ratio: "16:9" }): Promise<ImageReframeOutput> {
    return this.reframe(input);
  }

  /**
   * Reframe for ultrawide format (21:9)
   */
  async reframeForUltrawide(input: ImageReframeInput & { aspect_ratio: "21:9" }): Promise<ImageReframeOutput> {
    return this.reframe(input);
  }

  /**
   * Reframe with custom safety tolerance
   */
  async reframeWithSafetyTolerance(input: ImageReframeInput & { safety_tolerance: "1" | "2" | "3" | "4" | "5" | "6" }): Promise<ImageReframeOutput> {
    return this.reframe(input);
  }

  /**
   * Advanced reframing with all custom parameters
   */
  async reframeAdvanced(input: ImageReframeInput & {
    aspect_ratio: "21:9" | "16:9" | "4:3" | "3:2" | "1:1" | "2:3" | "3:4" | "9:16" | "9:21";
    guidance_scale: number;
    num_inference_steps: number;
    safety_tolerance: "1" | "2" | "3" | "4" | "5" | "6";
  ...
  /**
   * Get cost estimate for image reframing
   */
   ...
   */
  getRecommendedParameters(useCase: "social_media" | "mobile" | "widescreen" | "ultrawide" | "high_quality" | "fast" | "balanced" | "conservative") {
    const recommendations = {
      ...
      widescreen: { aspect_ratio: "16:9" as const, guidance_scale: 3.5, num_inference_steps: 30, safety_tolerance: "2" as const },
      ultrawide: { aspect_ratio: "21:9" as const, guidance_scale: 3.5, num_inference_steps: 30, safety_tolerance: "2" as const },
  ...
  getAspectRatioGuide(): Array<{ ratio: string; description: string; useCase: string; examples: string[] }> {
    return [
      { ratio: "21:9", description: "Ultrawide format", useCase: "Cinematic content, ultrawide displays", examples: ["movie trailers", "gaming content", "ultrawide monitors"] },
      { ratio: "16:9", description: "Widescreen format", useCase: "Standard video content, desktop displays", examples: ["YouTube videos", "presentations", "desktop backgrounds"] },
      { ratio: "4:3", description: "Standard format", useCase: "Traditional displays, older content", examples: ["legacy content", "presentations", "documentation"] },
  ...
  getGuidanceScaleGuide(): Array<{ range: string; effect: string; useCase: string }> {
    return [
      { range: "1.0-2.5", effect: "Very loose adherence to original", useCase: "Creative reinterpretation" },
      { range: "2.5-3.5", effect: "Balanced reframing", useCase: "Standard reframing" },
      { range: "3.5-4.5", effect: "Strong adherence to original", useCase: "Faithful reframing" },
      { range: "4.5-6.0", effect: "Very strong adherence", useCase: "Maximum preservation" },
      { range: "6.0-10.0", effect: "Extreme adherence", useCase: "Experimental use" }
    ];
  }
   ...
   */
  validateInput(input: ImageReframeInput): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
   ...
   */
  getEffectDescription(aspectRatio: string, guidanceScale: number, inferenceSteps: number, safetyTolerance: string): string {
    ...
    return `Intelligent reframing to ${aspectDesc} format with ${guidanceDesc} adherence to original and ${qualityDesc} quality processing (guidance: ${guidanceScale}, steps: ${inferenceSteps}, safety: ${safetyTolerance})`;
  }
   ...
   */
  getOptimalSettings(contentType: "portrait" | "landscape" | "social_media" | "mobile" | "widescreen" | "mixed") {
    const settings = {
      portrait: { aspect_ratio: "9:16" as const, guidance_scale: 3.5, num_inference_steps: 30, safety_tolerance: "2" as const },
      landscape: { aspect_ratio: "16:9" as const, guidance_scale: 3.5, num_inference_steps: 30, safety_tolerance: "2" as const },
      social_media: { aspect_ratio: "1:1" as const, guidance_scale: 3.5, num_inference_steps: 30, safety_tolerance: "2" as const },
      mobile: { aspect_ratio: "9:16" as const, guidance_scale: 3.5, num_inference_steps: 30, safety_tolerance: "2" as const },
      widescreen: { aspect_ratio: "16:9" as const, guidance_scale: 3.5, num_inference_steps: 30, safety_tolerance: "2" as const },
...
export const falAiImageReframe = new FalAiImageReframeExecutor();
