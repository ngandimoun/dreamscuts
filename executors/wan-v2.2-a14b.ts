...
// Types for the Wan v2.2-a14b model
export interface WanV22A14BInput {
  prompt: string;
...
export interface WanV22A14BOutput {
  image: {
    url: string;
    content_type?: string;
    file_name?: string;
...
export interface WanV22A14BError {
  error: string;
  message: string;
  code?: string;
}
...
// Model configuration
const MODEL_ENDPOINT = "fal-ai/wan/v2.2-a14b/text-to-image";
...
/**
 * Wan v2.2-a14b Image Generation Executor
 * 
 * This executor provides a comprehensive interface for generating ultra-high quality,
 * photorealistic images using the Wan v2.2-a14b model through the fal.ai API. It's
 * optimized for exceptional image quality with the 14B parameter architecture,
 * providing superior prompt understanding and fine-grained visual detail.
 */
export class WanV22A14BExecutor {
  ...
  /**
   * Generate images using the Wan v2.2-a14b model
   * @param input - The input parameters for image generation
   * @returns Promise<WanV22A14BOutput> - The generated image and metadata
   */
  async generateImages(input: WanV22A14BInput): Promise<WanV22A14BOutput> {
    try {
      // Validate input
   ...
   * @returns Promise<WanV22A14BOutput> - The generated image and metadata
   */
  async getQueueResult(requestId: string): Promise<WanV22A14BOutput> {
    try {
   ...
   * @returns Promise<WanV22A14BOutput[]> - Array of results for each prompt
   */
  async generateMultipleImages(
    prompts: string[],
    imageSize?: WanV22A14BInput["image_size"]
  ): Promise<WanV22A14BOutput[]> {
    const results: WanV22A14BOutput[] = [];
    ...
    for (const prompt of prompts) {
      try {
        ...
        results.push({
          image: { url: "" },
          error: (error as WanV22A14BError).message,
        });
      }
    }
   ...
   * @returns Promise<WanV22A14BOutput[]> - Array of results for each size
   */
  async generateWithImageSizes(
    input: Omit<WanV22A14BInput, "image_size">,
    imageSizes: WanV22A14BInput["image_size"][]
  ): Promise<WanV22A14BOutput[]> {
    const results: WanV22A14BOutput[] = [];
   ...
   * @param input - The input parameters for image generation
   * @returns Promise<WanV22A14BOutput> - The generated image with enhanced prompts
   */
  async generateWithPromptExpansion(input: WanV22A14BInput): Promise<WanV22A14BOutput> {
   ...
   * @param input - The input parameters for image generation
   * @returns Promise<WanV22A14BOutput> - The generated ultra-high quality image
   */
  async generateUltraHighQuality(input: WanV22A14BInput): Promise<WanV22A14BOutput> {
    return this.generateImages({
      ...input,
   ...
   * @param input - The input parameters for image generation
   * @returns Promise<WanV22A14BOutput> - The generated image with no acceleration
   */
  async generateWithNoAcceleration(input: WanV22A14BInput): Promise<WanV22A14BOutput> {
    return this.generateImages({
      ...input,
      acceleration: "none",
    });
  ...
  calculateImagesPerDollar(budget: number): number {
    return Math.floor(budget / 0.025);
  }
   ...
   */
  private validateInput(input: WanV22A14BInput): void {
   ...
   */
  private isValidImageSize(imageSize: any): imageSize is WanV22A14BInput["image_size"] {
   ...
   */
  private mergeWithDefaults(input: WanV22A14BInput): WanV22A14BInput {
    return {
      ...DEFAULT_CONFIG,
      ...input,
    };
   ...
   */
  private handleError(error: any): WanV22A14BError {
    if (error.message) {
      return {
...
export const wanV22A14BUtils = {
   ...
   * @param subject - Scientific subject
   * @param type - Visualization type
   * @returns Formatted prompt
   */
  createScientificPrompt(subject: string, type: string = "scientific"): string {
    return `${type} visualization of ${subject}, detailed, accurate, high quality, photorealistic detail, scientific precision, ultra-fine detail`;
  },
   ...
   */
  getRecommendedImageSize(useCase: string): WanV22A14BInput["image_size"] {
    const recommendations: Record<string, WanV22A14BInput["image_size"]> = {
...
export default WanV22A14BExecutor;
