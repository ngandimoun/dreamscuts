...
// Model endpoint
const MODEL_ENDPOINT = 'seedream-4';
...
// Output interface
export interface Seedream4Output {
  video: {
    url: string;
    content_type: string;
    file_name?: string;
...
// Error interface
export interface Seedream4Error {
  error: string;
  message: string;
  details?: any;
}
...
// Result type
export type Seedream4Result = Seedream4Output | Seedream4Error;

// ============================================================================
// MAIN EXECUTOR CLASS
// ============================================================================

export class Seedream4Executor {
  ...
  /**
   * Generate video using Seedream 4 model via fal.subscribe
  ...
  async generateVideo(
    ...
    try {
      ...
      return result.data as Seedream4Output;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
      throw new Error(`Seedream 4 video generation failed: ${errorMessage}`);
    }
  }
  ...
  /**
   * Generate vintage-style video (like the Gotham example)
   */
  async generateVintageVideo(
    prompt: string,
    vintageStyle: string = '2004 VGA analog selfie',
    options?: {
      aspect_ratio?: string;
      duration?: string;
      resolution?: string;
    ...
    return await this.generateVideo({
      prompt: prompt,
      style: `photographic ${vintageStyle}`,
      aspect_ratio: options?.aspect_ratio as any || '4:5',
      duration: options?.duration as any || '5s',
      resolution: options?.resolution as any || '1080p',
      negative_prompt: options?.negative_prompt || 'logos, text, extra limbs, smooth skin, HDR, modern phone'
    });
  }
  ...
  async submitVideoRequest(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
      throw new Error(`Seedream 4 request submission failed: ${errorMessage}`);
    }
  }
   ...
   */
  async getRequestResult(requestId: string): Promise<Seedream4Output> {
    try {
   ...
   */
  private validateInput(input: Seedream4Input): void {
    if (!input.prompt) {
...
/**
 * Create a new Seedream4Executor instance
 */
export function createSeedream4Executor(apiKey: string): Seedream4Executor {
  return new Seedream4Executor(apiKey);
}

/**
 * Quick video generation function
 */
export async function generateSeedream4Video(
  apiKey: string,
  prompt: string,
  options?: {
    style?: string;
  ...
  const executor = new Seedream4Executor(apiKey);
...
/**
 * Generate vintage-style video
 */
export async function generateVintageVideo(
  apiKey: string,
  prompt: string,
  vintageStyle?: string,
  options?: {
  ...
  const executor = new Seedream4Executor(apiKey);
  return await executor.generateVintageVideo(prompt, vintageStyle, options);
}

export default Seedream4Executor;
